...
for geneSetName in $(cat list1.txt)
#$(ls ../PvalueGeneList/genesListAllPaper/|grep "txt")
do
echo ${geneSetName}
...
#################
#ADD GENE SET
rFile=scriptSCZ2017usingGeneSetForEachGeneSet.R
rFile=scriptAUTfromTADAusingGeneSetForEachGeneSet.R

OUT_DATA_DIR=W_SCZgeneSetEachGeneSet
OUT_DATA_DIR=W_AUTgeneSetEachGeneSet
...
/hpc/users/nguyet26/R/bin/R CMD BATCH --no-save --no-restore \
    "--args ${ii} ${jj} ${kk} ${iThin} ${iChain} ${annotationType} ${annotationType2} ${MM} ${CURRENT_DIR} ${lowerHyperGamma} ${annotationType3} ${OUT_DATA_DIR} ${nGroupDN} ${nGroupCC} ${adjustHyperBeta} ${misType} ${sigmaPrior} ${geneSetName} ${swapData} ${singleClass}" ${CURRENT_DIR}/${rFile} \
$outDir/${rFile}.chain.${iChain}.${jj}.$kk.${changeFile}.${annotationType}.${annotationType2}.${annotationType3}.${MM}.${geneSetName}.Rout
