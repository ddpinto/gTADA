...
for geneSetName0 in $(ls ../PvalueGeneList/genesListAllPaper/*txt|awk -F"/" '{print $NF}')
do
...
bsub -P acc_psychgen -q premium -n $iChain -W 5:35 -R "rusage[mem=${MM}]" -R "span[hosts=1]" \
...
/hpc/users/nguyet26/R/bin/R CMD BATCH --no-save --no-restore \
    "--args $ii $jj $kk $iThin $iChain $annotationType $annotationType2 ${MM} ${CURRENT_DIR} ${lowerHyperGamma} ${annotationType3} ${OUT_DATA_DIR} ${nGroupDN} ${nGroupCC} ${adjustHyperBeta} ${swapData} ${sigmaPrior} ${kF} ${geneSetName0}" ${CURRENT_DIR}/${rFile} \
$outDir/${rFile}.chain.${iChain}.${jj}.$kk.${changeFile}.${annotationType}.${annotationType2}.${annotationType3}.${MM}.Rout

done

done < listGroupEPIandID.txt2CHD 
#listGroupEPIandID.txt2damagingIDandEPI
#listGroupEPIandID.txt2damagingDD
#listGroupEPIandID.txt2damagingSCZ
#listGroupEPIandID.txt2
#listGroup.txt1o
