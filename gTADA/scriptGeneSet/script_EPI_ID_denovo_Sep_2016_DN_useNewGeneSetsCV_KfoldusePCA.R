...
if ((annotationType == "lof_AST_TADA") | (annotationType == "lof_AST_allN5122")){
...
#g0 <- rbind(g0, t(c(1, "Summary.DLPFC_preds2_META1.hgnc_id.txt")))
...
##Get PCs
bpca <- prcomp(geneSetM, scale = TRUE)

head(bpca$rotation, 2)
#pdf("~/www/Test/xem1.pdf")
plot(bpca$x[, 1], bpca$x[, 2])

##START to divide the data into 10-fold
sK <- floor(seq(1, dim(data)[1], length = 11))
if (sK[length(sK)] < dim(data)[1])
    sK[length(sK)] <- dim(data)[1]
sK <- data.frame(sK[-length(sK)], sK[-1])
sK[, 2] <- sK[, 2] - 1
sK[dim(sK)[1], 2] <- dim(data)[1]
sK

sK1 <- as.integer(sK[kF,])
sK2 <- sK1[1]:sK1[2]
...
geneSetM01 <- geneSetM[-sK2, ]
dim(geneSetM01)
geneSetM02 <- geneSetM[sK2, ]
dim(geneSetM02)
...
modelDNdata <- list(NN = dim(allDNData[-sK2,])[1], #Gene numbers
                                        K = 2, #Hypothesis numbers: should be default
                                        NCdn = 2, #Number of de novo classes
                                        Ndn = array(rep(N$dn, 2)), # Family numbers
                                        dataDN = array(allDNData[-sK2,]), # Denovo data
                                        mutRate = array(allMutData[-sK2, ]), # Mutation rates
...
#                    hyper2BetaDN = array(c(0.01, 0.01))
                  ...
                  Tgs = dim(geneSetM01)[2],
                  Ngs = dim(geneSetM01)[2],
                  geneSet = data.frame(geneSetM01),
